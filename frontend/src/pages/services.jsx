...
import ServiceUpdateModal from "../components/services/updateModal";
...
const Services = () => {
  ...
  const filteredServices =
    services?.filter((item) => {
      ...
      const fees = String(item?.service_fees ?? "");
      return name.includes(q) || fees.includes(q);
    }) ?? [];
  ...
  const onDelete = async () => {
    try {
      await axiosClient.delete(`/api/services/${selectedService?.id}`);
      const deletedServ = services.filter((s) => s?.id !== selectedService?.id);
  ...
  return (
    <div className="flex justify-center min-h-0 flex-1">
      <div className="w-full xl:w-[95%] flex flex-col overflow-hidden">
        <h2 className="text-2xl font-bold text-slate-800 dark:text-slate-100 mb-4">Services</h2>
        ...
        {showFilter && (
          <div className="mb-4 rounded-lg border border-slate-200 dark:border-slate-600 bg-slate-50/50 dark:bg-slate-800/50 px-4 py-3 text-sm text-slate-600 dark:text-slate-400">
            No filters available for services yet.
          </div>
        )}
              ...
              <tbody>
                {filteredServices.map((item) => (
                  ...
                  >
                    ...
                    <td className="py-3 px-4">{item.service_fees}</td>
                    {user?.role === "admin" && (
